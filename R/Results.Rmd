...
*3.1. Fit to data*\
Three single-species and 13 multi-species age-structured models were fit to fishery and survey data for walleye pollock, arrowtooth flounder, Pacific cod, and Pacific halibut data in the GOA. Models that included predation mortality, but excluded predation from Pacific halibut led to the lowest AIC for all time-series (Tables 5-6). For the long and medium time-series models (Models 1-8), single-species models provided the best fit overall to the composition data (Table 6). Alternatively, for the long and medium time-series models (Models 9-11), models that included predation provided the best fit to the index data (Table 6). For short-time series models (Models 12-16), the single-species model (Model 12) led to the best fit to the index data (model 12), while the model that included predation (Model 13) led to the best fit to the composition data and the two models had a comparable AIC.

*3.2. Comparison of alternative time series length from individual models*\
The long-, medium-, and short-term multi-species models fit with different Pacific halibut numbers-at-age had similar trends in population quantities for some time periods (Supplementary Figs. 3-5). However, the long-term models differed slightly between short- and medium-term models. For example, recent estimates of biomass and spawning stock biomass of walleye pollock and arrowtooth flounder were estimated to be slightly greater in 1996 from long time-term multi-species models (Supplementary Figs. 3-4). While we estimated that arrowtooth flounder consumed the most biomass of walleye pollock in all multi-species models, the short-term models (Models 13-16) estimated that arrowtooth flounder consumed more biomass of walleye pollock than the long- and medium-term models (Supplementary Fig. 7). Cannibalism of arrowtooth flounder also differed among the long-, medium-, and short-term models (Supplementary Fig. 7) and total natural mortality (M1 + M2) was estimated to be lower in the long- versus short-term models for walleye pollock (Supplementary Fig. 9).


*3.3. Comparison of alternative halibut distributions from individual models*\
For the long time-series models, estimates of biomass, spawning stock biomass, and recruitment varied little among assumptions regarding historical Pacific halibut distribution in the GOA pre-1993 (Supplementary Figs. 3-5). 15th, 50th, or 85th quantile of adult halibut in area 3 between 1993 and 2018 was assumed for pre-1993 abundance in area 3 led to little difference in derived quantities. Similarly, using the numbers-at-age from the Pacific halibut areas-as-fleets and coast-wide models provided very similar trends in derived quantities for all individual models.


*3.4. Time series of derived quantities from ensemble models*\
Ensemble models were constructed by averaging individual model results across predation types ignoring AIC (no predation, no halibut predation, halibut predation vis absolute or relative numbers at age; Table 5). There were relatively large differences in estimates of biomass, spawning stock biomass, and recruitment of walleye pollock and arrowtooth flounder relative to the single-species ensemble model for ensemble models that included predation and estimated residual natural mortality (Figs. 2-3). For example, across all years, ensemble models that included predation estimated biomass of pollock to be on average `r round(mean(biom_diff_avg[[1]][1,1:23]), 2)` (Avg 2), `r round(mean(biom_diff_avg[[2]][1,1:23]), 2)` (Avg 3), and `r round(mean(biom_diff_avg4[1,1:23]), 2)` (Avg 4) times greater than estimates from the single species ensemble model. Similarly, spawning stock biomass of walleye pollock was estimated to be `r round(mean(ssb_diff_avg[[1]][1,1:23]), 2)` (Avg 2), `r round(mean(ssb_diff_avg[[2]][1,1:23]), 2)` (Avg 3), and `r round(mean(ssb_diff_avg4[1,1:23]), 2)` (Avg 4) times greater across all years when predation was included. These differences were also evident when comparing individual multi-species models to single-species models (Models 1-16; Supplementary Figs. 3-4). Estimates of arrowtooth flounder biomassand recruitment were larger in scale, but similar in trend, across models that included predation when compared to the single-species ensemble model. However, spawning stock biomass of arrowtooth flounder was lower across models that included predation. Estimates of biomass and spawning stock biomass of Pacific cod from the ensemble models were less sensitive to the inclusion of predation, although estimates of recruitment of Pacific cod were greater in ensemble models that included predation (Figs. 2-3). 

*3.5. Biomass consumed*\
Biomass consumed as prey of each species included in the model differed across model variants. When predation from Pacific halibut was included in the multi-species ensemble models as absolute numbers-at-age (Avg 3), biomass consumed as prey was relatively higher for all species (Fig. 4), leading to slightly lower estimates of biomass, spawning stock biomass, and recruitment. Trends were similar between multi-species ensemble models that excluded Pacific halibut predation (Avg 2) and treated Pacific halibut as relative numbers-at-age (Avg 4). The ensemble multi-species model that excluded predation from Pacific halibut (Avg 2) estimated that on average `r round(mean(colSums(mod_list_avg[[2]]$quantities$B_eaten[1,1,,1:23])))` mt of pollock, `r round(mean(colSums(mod_list_avg[[2]]$quantities$B_eaten[2,1,,1:23]) + colSums(mod_list_avg[[2]]$quantities$B_eaten[2,2,,1:23])))` mt of arrowtooth flounder, and `r round(mean(colSums(mod_list_avg[[2]]$quantities$B_eaten[3,1,,1:23])))` mt of Pacific cod was consumed annually by predators included in the model (Fig. 4). However, the ensemble multi-species model that included predation from Pacific halibut as absolute numbers-at-age (Avg 3) estimated that on average `r round(mean(colSums(mod_list_avg[[3]]$quantities$B_eaten[1,1,,1:23])))` mt of pollock, `r round(mean(colSums(mod_list_avg[[3]]$quantities$B_eaten[2,1,,1:23]) + colSums(mod_list_avg[[3]]$quantities$B_eaten[2,2,,1:23])))` mt of arrowtooth flounder, and `r round(mean(colSums(mod_list_avg[[3]]$quantities$B_eaten[3,1,,1:23])))` mt of Pacific cod was consumed annually by predators included in the model (Fig. 4). When Pacific halibut was included as relative numbers-at-age we estimated that  `r round(mean(colSums(mod_list_avg[[4]]$quantities$B_eaten[1,1,,1:23])))` mt of pollock, `r round(mean(colSums(mod_list_avg[[4]]$quantities$B_eaten[2,1,,1:23]) + colSums(mod_list_avg[[4]]$quantities$B_eaten[2,2,,1:23])))` mt of arrowtooth flounder, and `r round(mean(colSums(mod_list_avg[[4]]$quantities$B_eaten[3,1,,1:23])))` mt of Pacific cod was consumed annually. Temporal trends in biomass consumed as prey were similar across ensemble models (Fig. 4).
...
*3.6. Comparison of fixed vs relative halibut abundance*\
